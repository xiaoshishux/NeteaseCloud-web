        ...
        <div>
          ...
          <div class="icon">
            <span class="iconfont icon-yinle1">
              <i>{{ singerInfo.musicSize }}</i>
            </span>
            <span class="iconfont icon-zhuanjiguangpan">
              <i>{{ singerInfo.albumSize }}</i>
            </span>
            <span class="iconfont icon-MV">
              <i>{{ singerInfo.mvSize }}</i>
            </span>
          </div>
        </div>
        <el-card>
          <el-tabs type="border-card" :stretch="true" @tab-click="toggle">
            ...
            <el-tab-pane label="热门作品">
              ...
              >
                ...
                <el-table-column label="歌曲" width="400px" class="s_name">
                  <template #default="scope">
                    <span class="l_s_name" @click="getSongDet(scope.row.id)">{{
                      scope.row.name
                    }}</span>
                  </template>
                </el-table-column>
                ...
                <el-table-column label="专辑">
                  <template #default="scope">
                    <span class="l_s_name">{{ scope.row.al.name }}</span>
                  </template>
                </el-table-column>
              </el-table>
            </el-tab-pane>
            ...
            <el-tab-pane label="相关MV">
              <ul class="u_mv">
                <li
                  v-for="item in mvs"
                  :key="item.id"
                  @click.prevent="saveMvId(item.id)"
                >
                  <div class="mv_cover">
            ...
            <el-tab-pane label="艺人介绍">
              <div>
                ...
                <ul class="desc">
                  <li
                    v-for="(item1, index1) in this.singerDesc.introduction"
                    :key="index1"
                  >
                    <p class="s_desc">{{ item1.ti }}</p>
                    <p
                      class="intr"
                      v-for="(item2, index2) in item1.txt"
                      :key="index2"
                    >
                      {{ item2 }}
                    </p>
